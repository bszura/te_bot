...
const channelsWithIntervals = {
  '1328104421812338811': 1 * 61 * 60 * 1000,
  '1286351421691793466': 1 * 61 * 60 * 1000,
};
    ...
    if (!partneringUsers.has(message.author.id)) {
      ...
      if (userAd === null) {
        ...
        if (reply && !reply.first().content.toLowerCase().includes('nie')) {
          ...
          const notificationUser = await client.users.fetch('1369745499304890601');
          await notificationUser.send(`Wymagane dołączenie na serwer:\n${userAd}`);
        }

        const guild = client.guilds.cache.get('1368678832680210512');
        if (!guild) {
